...
RequestExecution.prototype._sendOnConnection = function () {
  var self = this;
  this._operation =
    this._connection.sendStream(this._parent.request, this._parent.requestOptions, function responseCb(err, response) {
      if (self._cancelled) {
      ...
      if (response.schemaChange) {
        return self.client.handleSchemaAgreementAndRefresh(self._connection, response.schemaChange, function schemaCb(){
...
RequestExecution.prototype._getResultSet = function (response) {
  return new types.ResultSet(response, this._host.address, this._parent.triedHosts, this._parent.request.consistency);
};
...
RequestExecution.prototype._handleError = function (err) {
  this.triedHosts[this._host.address] = err;
  ...
  if (!decisionInfo || decisionInfo.decision === retry.RetryPolicy.retryDecision.rethrow) {
    if (this._parent.request instanceof requests.QueryRequest || this._parent.request instanceof requests.ExecuteRequest) {
      err['query'] = this._parent.request.query;
    }
...
RequestExecution.prototype._getDecision = function (err) {
  var operationInfo = {
    query: this._parent.request && this._parent.request.query,
    ...
    handler: this,
    request: this._parent.request,
    retryOnTimeout: false
  };
  var self = this;
  function onRequestError() {
    return self._parent.retryPolicy.onRequestError(operationInfo, self._parent.request.consistency, err);
  }
  if (err.isSocketError) {
...
RequestExecution.prototype._retry = function (consistency, useCurrentHost) {
  ...
  if (typeof consistency === 'number') {
    // TODO: Clone
    this._parent.request.consistency = consistency;
  }
